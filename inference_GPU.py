import tensorrt as trt
import torch
import numpy as np
from PIL import Image
import torch.onnx
import tensorrt as trt
import pycuda.driver as cuda
import pycuda.autoinit
import torchvision.transforms as transforms
import time



# Load the optimized TensorRT engine
engine_path = "~/inference_k/engine_model.trt"
with open(engine_path, "rb") as f:
    engine_data = f.read()

runtime = trt.Runtime(trt.Logger(trt.Logger.WARNING))
engine = runtime.deserialize_cuda_engine(engine_data)

timeStamp=time.time()
# Create an execution context to perform inference
with engine.create_execution_context() as context:
    # Allocate buffers for input and output
    inputs, outputs, bindings = [], [], []
    for binding in engine:
        size = trt.volume(engine.get_binding_shape(binding)) * engine.max_batch_size
        dtype = trt.nptype(engine.get_binding_dtype(binding))
        host_mem = cuda.pagelocked_empty(size, dtype)
        device_mem = cuda.mem_alloc(host_mem.nbytes)

        bindings.append(int(device_mem))
        if engine.binding_is_input(binding):
            inputs.append((host_mem, device_mem))
        else:
            outputs.append((host_mem, device_mem))
    image_path = "~/inference_k/img.jpg"
    
    #lread= time.time()-timeStamp

      
    # Load the image using PIL
    image = Image.open(image_path)

    # Convert PIL image to a NumPy array
    image_np = np.array(image)

    # Define the transformations (resize and normalize)
    preprocess_transform = transforms.Compose([
          transforms.ToPILImage(),        # Convert NumPy array back to PIL image
          transforms.Resize((112, 112)),  # Resize the image to (112, 112) as required
          transforms.ToTensor(),          # Convert PIL image to PyTorch tensor
          transforms.Normalize(mean=(0.5, 0.5, 0.5), std=(0.5, 0.5, 0.5))  # Normalize the image
      ])

    input_data = preprocess_transform(image_np).unsqueeze(0)
    #lproc=time.time() - lread
    #print(input_data)
    # Prepare your input data (e.g., load images, preprocess, etc.)
    # And copy the input data to the GPU
    # For example:
    #input_data = preprocess_input(your_input_data)  # Define your preprocessing function
    inputs[0][0][:] = input_data.ravel()

    # Transfer input data to the GPU (PyTorch CUDA)
    #inputs[0][1].copy_(torch.from_numpy(inputs[0][0]))

    input_data_np = inputs[0][0]
    input_data_tensor = torch.from_numpy(input_data_np)
    input_data_gpu = input_data_tensor.to("cuda")

    # Run inference
    context.execute_v2(bindings)

    # Transfer output data back from the GPU (PyTorch CUDA)
    #output_data = outputs[0][1].cpu().numpy()
    cuda.memcpy_dtoh(outputs[0][0], outputs[0][1])  # Copy data from GPU to CPU

# Convert the output tensor to a NumPy array
    output_data_np = outputs[0][0]

    #lout=time.time()-lproc
    # Post-process the output data if needed
    # For example:
    #output_results = postprocess_output(output_data)  # Define your postprocessing function

# Your output_results now contains the inference results after the optimization.
# You can use these results for further processing, analysis, or visualization.

#print(lread)
#print(lproc)
#print(lout)